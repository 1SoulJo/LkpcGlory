...
class MeditationAdapterV2(val fragment: MeditationFragment) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private val _typeItem: Int = 0
    private val _typeLoading: Int = 1

    var isLoading: Boolean = false

    var meditations: List<MeditationV2?> = mutableListOf()
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ItemViewHolder) {
            ...
            holder.date.text = meditation.scheduledDate
//            holder.date.text = newFormat.format(dateFormat.parse(meditation.scheduledDate!!)!!)

            val db = NoteDatabase.getDatabase(context = holder.itemView.context)
            db.noteDao().loadByContentId(meditation.id?.id!!).observe(
